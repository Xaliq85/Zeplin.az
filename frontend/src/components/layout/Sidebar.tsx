...
type MenuItem = { label: string; icon: React.ReactNode; to: string; badgeKey?: string }
...
const menuByRole: Record<Role, MenuItem[]> = {
  ...
  seller: [
    ...
    { label: 'Sifarişlər', icon: <ShoppingCart size={18} />,    to: '/seller/orders', badgeKey: 'pending_orders' },
...
function usePendingStats(role: Role) {
  return useQuery({
...
export default function Sidebar() {
  ...
  const { data: stats } = usePendingStats(user.role)
        ...
        {menu.map((item) => {
          ...
          return (
            ...
            >
              ...
              {badge > 0 && (
                <span className="min-w-[20px] h-5 px-1 rounded-full bg-gold text-navy text-xs font-chakra font-bold flex items-center justify-center">
                  {badge > 99 ? '99+' : badge}
                </span>
              )}
            </NavLink>
